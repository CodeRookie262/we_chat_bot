...
// 管理群组列表
const roomList = config.room.roomList //名称,是一个数组
  ...
  return async function onMessage(msg) {
    ...
    if (msg.type() == Message.Type.Text) {
      ...
      if (msg.room()) {
        ...
        // console.log('这是一条群聊',await room.topic(),roomList.indexOf(roomName))
        // if(~roomList.indexOf(roomName)){
        //   console.log('这个是我的群')
        //   // 请求机器人接口回复
...
async function isRoomName(bot, msg) {
  // 回复信息为管理的群聊名
  if (roomList.some(v => v == msg.text())) {
    // 通过群聊id获取到该群聊实例
    const room = await bot.Room.find({topic: '前端切图交流群'})
